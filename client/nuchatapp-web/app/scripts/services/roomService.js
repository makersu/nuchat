function RoomService($q, User, LBSocket, FriendService, $rootScope, $checkFormat, $utils, $filter, $timeout, $compile, METATYPE, $ChatboxTags) {
	console.log('RoomService');
	var DEBUG = false;
	var rooms = {};
	var _unreadMessages = [];
	var _chattingRooms = []; // Room IDs
  ...
  function grouping(room) {
    ...
    $ChatboxTags.setItemList(room.viewMessages);
    room.tagList = $ChatboxTags.getTagList().tags;
  }
  ...
  function getAllGroups(room) {
	...
	function addRoom(room) {
    	...
    	// $timeout(function() {
	    	...
	    	angular.forEach(room.joiners, function(joiner){
	    		// console.log(joiner)
	...
	function setChattingRoom(roomId) {
		console.log('setChattingRoom: '+roomId);
		_chattingRooms.push(roomId);
		// _filterBy = {};
	}

	function isChatting(roomId) {
		return _chattingRooms.indexOf(roomId) > -1;
	}

	function leaveChatting(roomId) {
		var idx = _chattingRooms.indexOf(roomId);
		(idx > -1) && _chattingRooms.splice(idx, 1);
	}
